...
router.post('/site', authenticate, async (req, res, next) => {
  try {
    const { scanned_data, project_name, lat, lng, notes } = req.body;
    const userId = req.user.id;
    const today = new Date().toISOString().split('T')[0];
    ...
    if (qrRows.length === 0) {
      return res.status(400).json({ error: 'Invalid Site QR Code. Attendance rejected.' });
    }

    const qr = qrRows[0];
    if (qr.status !== 'active') {
      return res.status(400).json({ error: `Site QR Code "${qr.name}" is INACTIVE. Attendance rejected.` });
    }
    if (qr.expiry_date && new Date(qr.expiry_date) < new Date()) {
      return res.status(400).json({ error: `Site QR Code "${qr.name}" has EXPIRED. Attendance rejected.` });
    }

    const dist = getDistanceMeters(parseFloat(lat), parseFloat(lng), parseFloat(qr.lat), parseFloat(qr.lng));
    const isWithinRadius = dist <= (qr.allowed_radius_meters || 300);
    const gpsStatus = isWithinRadius ? 'Inside Site' : 'Outside Site';
    ...
    const { rows } = await query(`
      INSERT INTO attendance_records (
        ...
        attendance_type, qr_code_id, qr_id_scanned, location_name, project_name,
        approval_status, is_late, gps_status, distance_meters, notes
      ) VALUES ($1, NOW(), $2, $3, 'present', 'site', $4, $5, $6, $7, 'pending', $8, $9, $10, $11)
      RETURNING *
    `, [
      userId, lat, lng, qr.id, qr.qr_id, qr.name, project_name || qr.project_name || 'On-Site Project',
      isLate, gpsStatus, Math.round(dist * 10) / 10, notes || null
    ]);
    ...
    for (const m of managers) {
      await query(`
        INSERT INTO notifications (user_id, title, message, type, link)
        VALUES ($1, 'New Site Attendance Submitted', $2, 'info', '/approvals')
      `, [m.id, `${req.user.name} submitted Site Attendance for ${project_name || qr.name}. Approval pending.`]);
    }
